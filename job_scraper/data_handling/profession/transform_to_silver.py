...
def clean_columns(df: DataFrame) -> DataFrame:
    only_needed_columns_df = df.select(['link','prof_name', 'item_brand', 'category3', 'category4', 'category6', 'variant'])
    ...
    result_df = renamed_columns_df \
        .with_columns(pl.col('salary_public').replace_strict({'salary publicised': True, 'salary confidential': False}))
...
def main():
    ...
    uri: str = f"postgresql://{POSTGRES_USERNAME}:{POSTGRES_PASSWORD}@localhost:5432/job_scraper"
    ...
    clean_df.write_database(table_name = "profession_data_silver", connection=uri, if_table_exists='replace')
